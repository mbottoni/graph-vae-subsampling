"""Paired-graph generators for dependence testing (Thread 2).

Each builds k pairs of graphs whose *generative parameters* are dependent with
strength rho, marginals held fixed. Given the parameters the graphs are
independent, so the dependence lives entirely at the parameter level — which is
what a summary statistic must expose. Moved out of the common library because
this construction is specific to the dependence-testing line.
"""
...
def correlated_er_pair(
    ...
    """A pair of ER graphs with edge-wise correlation rho.

    G1 ~ ER(n, p). For G2, each potential edge copies G1's indicator with
    probability rho and is resampled from Bernoulli(p) otherwise. Marginally
    G2 ~ ER(n, p); corr(A1_ij, A2_ij) = rho.
    """
...
def sbm_pair_series(
    ...
    signal is mostly orthogonal to that projection, so lambda_max should lose
    power while statistics that retain lambda_2 should not.
    """
    rng = np.random.default_rng(seed)
